...
export const PersonalBlogEditorModal = ({
    isOpen,
    onClose,
    onSave,
    initialData,
    ...
    const handleSave = async () => {
        ...
        setLoading(true)
        try {
            const isUnique = await personalBlogService.isSlugUnique(
                src,
                initialData?.key
            )
            if (!isUnique) {
            ...
            if (mode === 'create') {
                await personalBlogService.createPost(postData)
                toast('Post created successfully', { type: 'success' })
            } else if (initialData) {
                await personalBlogService.updatePost(initialData.key, postData)
